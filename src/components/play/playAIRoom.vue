    ...
    methods: {
      ...
      handicaps(coords){
        for(let i=0;i<coords.length;i++){
          let coord_str = Util.coordTostr(coords[i],Util.get_cols(this.game.board_size));
          this.game.to_AI_records.push(coord_str);
        }
      },
      ...
      getWebSocket1(e) {
        ...
        if (message.type == "play") {
          ...
          this.startTime();
          if(this.game.current_player==this.game.AI_player) {
            this.AI_play()
          }
          else{
      ...
      played(message){
        ...
        if(message.state=="fail")
          ...
          if(!this.game.if_try) {
            ...
            this.game.to_AI_records.push(message.coordStr);
            let send_message = {
              type: "play",
              content: {
      ...
      get_AI_type(name){
        if(name=="简单AI") return "weak";
        else if(name=="中等AI") return "middle";
        else return "start";
      },
      AI_play() {
        let that = this;
        if (!that.game_message.if_begin)
          return
        that.startTime()
        if (that.game.me_player == 3) return;
        let bot_name = ""
        if (that.game.AI_player == 1)
          bot_name = that.black.name;
        else
          bot_name = that.white.name;
        var params = {
          board_size: that.game.board_size,
          moves: that.game.to_AI_records,
        ...
        axios({
          ...
          if(that.if_show_compute_result||that.game.if_try){
            that.close_compute();
            that.untry_play();
          }
          let coord = Util.strTocoord(res.data.bot_move, Util.get_cols(that.game.board_size));
          let c = new window.JGO.Coordinate(coord.i, coord.j);
          that.$refs.goBoard.apply_move(c,true);
          that.game.records.push(Util.coordTostr(c));
          that.game.to_AI_records.push(Util.coordTostr(c))
          that.afterplay();
          ...
          that.socket1.send(JSON.stringify(message));
          that.startTime();
          that.$refs.goBoard.play("stone");
          }).catch(function (e) {
          console.log(e)
        })
      },
