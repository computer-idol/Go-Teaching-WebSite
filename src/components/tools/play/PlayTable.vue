      ...
      <div class="playTable-data">
        <div class="playTable-data-header">
          <p>
            ...
            <span>对局类型</span>
            <span>棋盘路数</span>
            <span style="width: 20%">黑方</span>
            <span>VS</span>
            <span style="width: 20%">白方</span>
            <span>棋局情况</span>
            <span>结果</span>
          </p>
        </div>
        <div class="playTable-data-main">
          <p v-for="(play,index) in copy_data" :key="play.roomid" @click="goto(index)" v-if="copy_data.length!=0">
            ...
            <span style="width: 20%;display: flex;flex-direction: row;justify-content: center">
              <img :src="play.black_player.photo" />
              <div>{{play.black_player.name}}</div>
              <div>{{play.black_player.level}}</div>
            </span>
            <span>
              <img src="../../../../static/imgs/play/win.png" v-if="play.winner==1"/>
              <img src="../../../../static/imgs/play/lose.png" v-else-if="play.winner==2"/>
              <img src="../../../../static/imgs/button/help_icon.png" v-else/>
            </span>
            <span style="width: 20%;width: 20%;display: flex;flex-direction: row;justify-content: center">
